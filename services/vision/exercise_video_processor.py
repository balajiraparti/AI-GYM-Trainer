...
from services.detectors.biceps_curl import BicepsCurlDetector
from services.detectors.lunges import LungesDetector
from services.detectors.pushup import PushUpDetector
from services.detectors.squat import SquatDetector
from services.detectors.shoulder_press import ShoulderPressDetector
import cv2
...
class VideoProcessorClass(VideoProcessorBase):
    def __init__(self):
        ...
        model_path=self._BASE_PATH / "ml_model" / "pose_landmarker_full.task"
        base_opion=python.BaseOptions(model_asset_path=str(model_path))
        options=vision.PoseLandmarkerOptions(base_options=base_opion,running_mode=vision.RunningMode.VIDEO,min_pose_detection_confidence=.7,min_pose_presence_confidence=.7,min_tracking_confidence=.7,output_segmentation_masks=False)

        self._landmarker=vision.PoseLandmarker.create_from_options(options)
        self._detectors={
            "Squats":SquatDetector(),
            "Push-ups":PushUpDetector(),
            "Bicep Curls (Dumbell)": BicepsCurlDetector()
    ...
    def _draw__overlays(self,img,metrics,ex_type):
        if ex_type=="Squats":
    ...
    def recv(self, frame):
        ...
        if result.pose_landmarks:
            ...
            if detector:
                ...
                self._draw__overlays(image,metrics,ex_type)
